...
fun main(args: Array<String>) {
  val fib = File("rosalind_fib.txt").readLines()[0];
  val list = fib.split(" ")
  val n = list[0].toLong()
  val k = BigInteger(list[1])
  Stream.iterate(Pair(BigInteger.ONE, BigInteger.ZERO), { pair -> Pair(pair.second * k, pair.first + pair.second) })
      .skip(n - 1L)
      .findFirst()
      .ifPresent{pair -> println(pair.first + pair.second)}
}